  ...
  init: async function (numberOracles) {
    ...
    flightSuretyApp.events
      ...
      .on('data', async log => {
        ...
        // store new flight
        const indexFlightKeys = await flightSuretyData.methods
          .indexFlightKeys()
          .call()
    ...
    flightSuretyApp.events.OracleReport().on('data', log => {
      const {
        event,
    ...
    flightSuretyApp.events.FlightProcessed().on('data', log => {
      const {
        event,
    ...
    flightSuretyApp.events.WithdrawRequest().on('data', log => {
      const {
        event,
    ...
    flightSuretyData.events.Paid().on('data', log => {
      const {
        event,
    ...
    flightSuretyData.events.Credited().on('data', log => {
      const {
        event,
    ...
    this.oracles = (await web3.eth.getAccounts()).slice(
      NUMBER_OF_ACCOUNTS - numberOracles
    )
    // register oracles
    const REGISTRATION_FEE = await flightSuretyApp.methods
      .REGISTRATION_FEE()
      .call()
